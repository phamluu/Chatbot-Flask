...
def handle_manage_intent(request):
    if request.method == 'POST':
        ...
        return redirect(url_for('chatbot_api.manage_intent'))
...
def handle_manage_intent_input(request, intent_id):
    if request.method == 'POST':
        ...
        return redirect(url_for('chatbot_api.manage_intent_input', intent_id=intent_id))
...
def handle_update_intent_input(id, request):
    ...
    db.session.commit()
    flash('Intent updated successfully!', 'success')
    return redirect(url_for('chatbot_api.manage_intent_input', intent_id=intent_input.intent_id))
...
def handle_manage_intent_response(request, intent_id):
    if request.method == 'POST':
        ...
        if response_text:
            intent_response = IntentResponse(response_text=response_text, intent_id=intent_id)
        ...
        return redirect(url_for('chatbot_api.manage_intent_response', intent_id=intent_id))
    intent_responses = IntentResponse.query.filter_by(intent_id=intent_id).all()
...
def handle_update_intent_response(id, request):
    ...
    intent_response.utterance = request.form.get('utterance')
    db.session.commit()
    flash('Intent updated successfully!', 'success')
    return redirect(url_for('chatbot_api.manage_intent_response', intent_id=intent_response.intent_id))
